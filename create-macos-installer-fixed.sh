...
if lsof -ti:3001 > /dev/null 2>&1; then
  ...
  # Start server
  "${NODE_CMD}" server.js > "${DATA_DIR}/server.log" 2>&1 &
  SERVER_PID=$!
  ...
  if ! lsof -ti:3001 > /dev/null 2>&1; then
    osascript -e "display dialog \"Failed to start server. Check ${DATA_DIR}/server.log for details.\" buttons {\"OK\"} default button \"OK\"" > /dev/null 2>&1
    exit 1
  fi
fi
