...
const Milestones = ({ milestones, onAdd, onEdit, onDelete, onCelebrate }) => {
  ...
  // Sort milestones by date (upcoming first, then past)
  ...
  const pastMilestones = sortedMilestones.filter(m => {
    const milestoneDate = new Date(m.date);
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    return milestoneDate < today;
    ...
    <div className="milestones-section">
      ...
      {upcomingMilestones.length > 0 && (
        <div className="milestones-group">
          <h4 className="milestones-group-title">Upcoming</h4>
          <div className="milestones-list">
            {upcomingMilestones.map((milestone) => (
              <div
                key={milestone.id}
                className={`milestone-item ${isToday(milestone.date) ? 'milestone-today' : ''} ${isUpcoming(milestone.date) ? 'milestone-upcoming' : ''}`}
              >
                ...
                <div className="milestone-actions">
                  {isToday(milestone.date) && !milestone.celebrated && (
                    <button
      ...
      {pastMilestones.length > 0 && (
        <details className="past-milestones">
          <summary>
          ...
          <div className="milestones-list">
            {pastMilestones.map((milestone) => (
              <div
                key={milestone.id}
                className={`milestone-item milestone-past ${milestone.celebrated ? 'milestone-celebrated' : ''}`}
              >
                <div className="milestone-content">
                  <div className="milestone-icon">
